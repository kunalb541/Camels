...
The exhaustive scan supports `{verdict}`. The nominal lower edge at `logMstar ~ 9.55` is not defensible as a sharp physical threshold. In the unfiltered CAMELS CV catalog it remains a recoverability threshold: a tiny gas/SFR-floor tail can suppress the low-mass L3-plus-gas score. After cleaning, gas mass contributes below `9.55`, including narrow low-mass and pre-edge bands. The previously established upper transition near `10.55` remains physically meaningful because sSFR drops, the quenched fraction rises, and BH mass rises there.
